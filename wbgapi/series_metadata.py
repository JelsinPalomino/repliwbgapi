...
def fetch(id, economies=[], time=[], db=None):
    ...
    Examples:
        for meta in wbgapi.series.metadata.fetch(['SP.POP.TOTL', 'NY.GDP.PCAP.CD']):
            print(meta)
    '''
    ...
    for row in w.metadata('source/{source}/series/{series}/metadata', ['series'], source=db, series=w.queryParam(id, 'series', db=db)):
        if economies:
            row.economies = {}
        ...
        if time:
            ...
            try:
                ...
                for row2 in w.metadata('sources/{source}/Series-Time/{series}/metadata', ['series'], source=db, series=st):
